<script lang="ts">
  ...
  // liveness, and global controls (transport mode, theme) regardless of which
  // session is selected.
  // What: Renders the "trusty-mpm" wordmark, a colored daemon-health dot bound
  // to the `daemonHealth` store, the TransportPill, and the ThemeToggle.
  // Test: Set `daemonHealth` to each of ok/connecting/error and assert the dot
  // color (green/amber/red) updates.
  import { daemonHealth } from '../stores/app';
